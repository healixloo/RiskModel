...
gene=read.table("risk.genes",sep="\t",header=F)
...
# volcano
pdf(file="RiskGenes_vol.pdf",height=5,width=5)
...
plot(as.numeric(as.vector(moduleDiffAll$logFC)), -log10(moduleDiffAll$fdr), xlab="logFC",ylab="-log10(fdr)",
     main="Volcano", ylim=c(0,yMax),xlim=c(-xMax,xMax),yaxs="i",pch=20, cex=0.8)+text(as.numeric(as.vector(moduleDiffAll$logFC)), -log10(moduleDiffAll$fdr),row.names(moduleDiffAll),cex=0.5,pos=4)
...
pdf(file="RiskGenes_heatmap.pdf",height=6,width=10)
pheatmap(hmExp, 
         annotation=Type, 
